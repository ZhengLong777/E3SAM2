"""Prepare EchoNet-Dynamic videos and annotations without edge targets.

Outputs include annotated PNG frames and full-cycle NumPy files under the
original TRAIN, VAL, and TEST splits.
"""
...
except ImportError:  # Support direct execution: python utils/preprocess_echonet.py
    from utils_contour import echonet_trace_to_mask
...
def parse_args() -> argparse.Namespace:
    ...
    _add_common_arguments(
        parser,
        default_output="data/EchoNet_Processed/Resized128_NoEdge",
    )
...
def preprocess_data(
    ...
    save_imgs: bool,
    resized_size: int,
    edge_generator: Callable[[torch.Tensor], torch.Tensor] | None = None,
) -> None:
    ...
    for video_index, row in enumerate(metadata.itertuples(index=False), start=1):
        ...
        if edge_generator is not None:
            cycle_payload["fnum_edge"] = frame_edges
        np.savez(cycle_annotation_dir / f"{name}.npz", **cycle_payload)
