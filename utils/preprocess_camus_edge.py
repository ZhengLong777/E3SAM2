"""Prepare CAMUS videos, LV masks, and Sobel-dilation edge targets.
...
except ImportError:  # Support direct execution: python utils/preprocess_camus_edge.py
    from compute_ef import compute_left_ventricle_volumes
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        "--output-dir",
        dest="output_dir",
        default="data/CAMUS_Processed/Edge_Sobel_Dilation_Size3",
        ...
        for view in VIEWS:
            ...
            with torch.no_grad():
                for frame_index, frame_mask in enumerate(masks):
                    binary_mask = (frame_mask == 1).astype(np.uint8)
                    edge = edge_generator(torch.from_numpy(binary_mask))
                    key = str(frame_index)
                    frame_masks[key] = binary_mask
                    frame_edges[key] = edge.cpu().numpy().astype(np.uint8)
